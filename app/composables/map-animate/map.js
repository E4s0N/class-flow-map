...
export class World extends Mini3d {
  ...
  createProvince() {
    ...
    this.eventElement = []
    province.mapGroup.children.map((group) => {
  ...
  getChildMapData(userData, callback) {
    let url = `/json/GeoMapData_CN/province/${userData.adcode}.json`
  ...
  showBadge(userData) {
    ...
    if (provinceData) {
      ...
      // 按工作地点分组人员
      const workGroups = {}
      provinceData.people.forEach((person) => {
        if (!workGroups[person.work]) {
          workGroups[person.work] = []
        }
      ...
      Object.keys(workGroups).forEach((work, index) => {
        ...
        // 添加工作地点标题
        contentHTML += `<div class="work-location">${work}</div>`
      ...
      // 创建标牌
      let label = this.label3d.create('', 'badges-label', true)
      label.init(contentHTML, position)
      this.label3d.setLabelStyle(label, 0.1, 'x')
      label.setParent(this.badgeGroup)
      label.show()

      // 保存当前显示的标牌引用
      this.currentVisibleBadge = label
    }
  }

  // 隐藏当前显示的标牌
  hideBadge() {
    if (this.currentVisibleBadge) {
      this.currentVisibleBadge.hide()
      this.currentVisibleBadge = null
    }
  }
